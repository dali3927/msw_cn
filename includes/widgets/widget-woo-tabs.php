...
class Woo_Tabs extends WP_Widget {
   ...
   function widget($args, $instance) {        
	   ...
	   $subscribe = ''; if ( array_key_exists( 'subscribe', $instance ) ) $subscribe = $instance['subscribe']; 
       ?>  
 		...
 		<div id="tabs">
            ...
            <ul class="wooTabs">
                ...
                <?php } elseif ( $order == "subscribe" && !$subscribe == "on") { ?><li class="subscribe"><a href="#sub"><?php _e('Subscribe', 'woothemes'); ?></a></li>
                <?php } ?>
                <?php if (!$pop == "on") { ?><li class="popular"><a href="#tab-pop"><?php _e('热门文章', 'woothemes'); ?></a></li><?php } ?>
                <?php if ($order <> "latest" && !$latest == "on") { ?><li class="latest"><a href="#tab-latest"><?php _e('最新文章', 'woothemes'); ?></a></li><?php } ?>
                <?php if ($order <> "comments" && !$comments == "on") { ?><li class="comm"><a href="#tab-comm"><?php _e('评论', 'woothemes'); ?></a></li><?php } ?>
                <?php if ($order <> "tags" && !$tags == "on") { ?><li class="tags"><a href="#tab-tags"><?php _e('标签', 'woothemes'); ?></a></li><?php } ?>
            	<?php if ($order <> "subscribe" && !$subscribe == "on") { ?><li class="sub"><a href="#sub"><?php _e('订阅 ', 'woothemes'); ?></a></li><?php } ?>
            </ul>
            ...
            <div class="boxes box inside">
	            ...
	            <?php } elseif ( $order == "subscribe" && !$subscribe == "on") { ?>
    		    <div id="sub">
			        <ul>
						<li><h3><?php _e('Stay up to date', 'woothemes'); ?></h3><a href="<?php if ( get_option('woo_feedburner_url') <> "" ) { echo get_option('woo_feedburner_url'); } else { echo get_bloginfo_rss('rss2_url'); } ?>"><img src="<?php bloginfo('template_directory'); ?>/images/ico-rss.gif" alt="" /></a></li>
						...
						<li><a href="<?php echo get_option('woo_feedburner_id'); ?>" target="_blank"><?php _e('Subscribe to the feed via email', 'woothemes'); ?></a></li>
					</ul>            
    		    </div>
                <?php } ?>
                ...
                <?php if ($order <> "subscribe" && !$subscribe == "on") { ?>
    		    <div id="sub">
			        <ul>
						<li><h3><?php _e('Stay up to date', 'woothemes'); ?></h3><a href="<?php if ( get_option('woo_feedburner_url') <> "" ) { echo get_option('woo_feedburner_url'); } else { echo get_bloginfo_rss('rss2_url'); } ?>"><img src="<?php bloginfo('template_directory'); ?>/images/ico-rss.gif" alt="" /></a></li>
						<li><a href="<?php if ( get_option('woo_feedburner_url') <> "" ) { echo get_option('woo_feedburner_url'); } else { echo get_bloginfo_rss('rss2_url'); } ?>"><?php _e('Subscribe to the RSS feed', 'woothemes'); ?></a></li>
   ...
   function form($instance) {                
       ...
       <p>
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name('order'); ?>" class="widefat" id="<?php echo $this->get_field_id('order'); ?>">
                ...
                <option value="subscribe" <?php if($order == "subscribe"){ echo "selected='selected'";} ?>><?php _e('Subscribe', 'woothemes'); ?></option>
            </select>
        </p>
	   ...
	   <p>
	       <input id="<?php echo $this->get_field_id('subscribe'); ?>" name="<?php echo $this->get_field_name('subscribe'); ?>" type="checkbox" <?php if($subscribe == 'on') echo 'checked="checked"'; ?>><?php _e('Subscribe', 'woothemes'); ?></input>
       </p>  
       <?php 
   }
